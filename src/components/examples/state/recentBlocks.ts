import { block, collections, state } from "@typeberry/lib";
import { filledHash } from "../objects/helpers";
import type { ClassInstance } from "../types";

const makeReported = (seeds: number[]) => {
  return collections.HashDictionary.fromEntries(
    seeds.map((seed) => {
...
export const recentBlocksExample: ClassInstance<typeof state.RecentBlocks> = state.RecentBlocks.create({
